...
class SlantedEdge(edges.BaseEdge):
    ...
    def __call__(self, length, **kw):
        angle = abs(self.settings)
        if angle == 0:
            poly = [length]
        else:
            d = length * math.tan(math.radians(angle))
            l = length / math.cos(math.radians(angle))
...
![Tray Extra](static/samples/TrayInsert-3.jpg)
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--draft_angle", action="store", type=float, default=0.0,
            help="amount the walls angle outwards toward to top (above sizes are at the bottom)")
    ...
    def render(self):
        ...
        l = SlantedEdge(self, -self.draft_angle)
        r = SlantedEdge(self, self.draft_angle)
        ...
        for i in range(len(self.sx) - 1):
            e = [edges.SlottedEdge(self, self.sy, slots=0.5 * h), r, "e", l]
            self.rectangularWall(y, h, e, move="up")

        for i in range(len(self.sy) - 1):
            e = ["e", r, edges.SlottedEdge(self, self.sx[::-1], "e", slots=0.5 * h), l]
