...
"""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--coin_radius", action="store", type=float, default=25.0,
        ...
        self.argparser.add_argument(
            "--notch_depth", action="store", type=float, default=0.0,
    ...
    #   cx, cy    – disc centre in math coords
    ...
    # "inward" on screen = decreasing y on screen = increasing y in math coords.
    ...
    def _notch_circular(
        ...
        # Inward normal (rotated 90° CCW in math coords = toward disc centre).
        # "Inward" means decreasing y on screen = decreasing y in math coords
        # (since the notch is at the top: disc_r·sin(π/2) = +disc_r direction).
        # The inward direction from the chord toward the disc centre is -y in math.
        nx = uy        # normal pointing "inward" (toward disc centre in math)
        ny = -ux

        # Arc centre: move from chord midpoint inward by (arc_r - depth).
        arc_cx = mid_chord_x + nx * (arc_r - d)
        arc_cy = mid_chord_y + ny * (arc_r - d)

        # Angles from arc centre to each rim endpoint.
    ...
    def _notch_triangular(
        ...
        # Apex: on the disc centre-line, inward by 'depth' from the rim.
        apex_x = cx
        apex_y = cy + disc_r - depth          # math coords: higher y = toward top before flip
    ...
    def _notch_oval(
        ...
        rx = cx + disc_r * math.cos(a_right)
        ry = cy + disc_r * math.sin(a_right)
        # Deepest point: centre-line, inward by 'depth'.
        mid_x = cx
    ...
    def topDisc(self, move: str = "") -> None:
        ...
        # --- Common geometry ---
        # disc centre = (cx, cy); Y-flip applied by SVGSurface.
        # "top of disc" on screen  = (cx, cy − disc_r)
        #                          = (cx, cy + disc_r) in math coords.
        cx: float = r
        cy: float = r
