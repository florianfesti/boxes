...
b.render()
data = b.close()
out = Path(inspect.getfile(MyGenerator)).with_suffix('.svg')
...
```powershell
boxes --examples                # writes each .svg next to its .py file
```
...
- File: `boxes/generators/<subdir>/<stem>.jpg` (same stem as the `.py` file, 1200 px wide, ~square)
- Thumbnail: `<stem>-thumb.jpg` in the same directory (generated by `gen_sample_images.py`)
...
- Commit: `boxes/generators/path/<stem>*.jpg`
